// JXSML: Self-contained JPEG XL Decoder
...
// JXSML is a single-file library, but it consists of multiple subsystems which have to be
// separately `#include`d for the purpose of testing. therefore this file contains
// multiple "parts" which can be `#include`d by setting an appropriate `JXSML__PART_*` macro.
// `JXSML__RECURSING` is also used for templated code where the same code gets reused with
// slightly different parameters; setting it disables usual `JXSML__PART_*` behaviors.
#ifndef JXSML__RECURSING

#define JXSML_REVISION 2270 // (fractional gregorian year - 2000) * 100, with a liberal rounding

#define JXSML_DEBUG

#ifndef JXSML_FILENAME // should be provided if this file has a different name than `jxsml.h`
#define JXSML_FILENAME "jxsml.h"
#endif
...
#define JXSML__RECURSING (-1)
#define JXSML__PART_PLATFORM
#define JXSML__PART_API
#ifdef JXSML_IMPLEMENTATION
	#define JXSML__PART_STATE
	#define JXSML__PART_ERROR
	#define JXSML__PART_UTILITY
	#define JXSML__PART_ALIGNED_PTRS
	#define JXSML__PART_VIEW
	#define JXSML__PART_PLANE
	#define JXSML__PART_SOURCE
	#define JXSML__PART_CONTAINER
	#define JXSML__PART_BITS
	#define JXSML__PART_PREFIX_CODE
	#define JXSML__PART_HYBRID_INT
	#define JXSML__PART_ANS_CODE
	#define JXSML__PART_ENTROPY_CODE
	#define JXSML__PART_IMAGE_HEADER
	#define JXSML__PART_ICC
	#define JXSML__PART_MODULAR_TREE
	#define JXSML__PART_MODULAR_HEADER
	#define JXSML__PART_MODULAR_PREDICT
	#define JXSML__PART_MODULAR_TRANSFORM
	#define JXSML__PART_DEQUANT_MATRIX
	#define JXSML__PART_FRAME_HEADER
	#define JXSML__PART_DCT
	#define JXSML__PART_LF_GLOBAL
	#define JXSML__PART_LF_GROUP
	#define JXSML__PART_HF_GLOBAL
	#define JXSML__PART_PASS_GROUP
	#define JXSML__PART_DEQUANT
	#define JXSML__PART_FRAME
#endif // defined JXSML_IMPLEMENTATION
#ifdef __cplusplus
extern "C" {
#endif
#include JXSML_FILENAME
#ifdef __cplusplus
}
#endif
#undef JXSML__RECURSING

#endif // !defined JXSML__RECURSING

////////////////////////////////////////////////////////////////////////////////
// platform macros

#if defined JXSML__PART_PLATFORM && JXSML__RECURSING < 0
		...
		#define JXSML__HAS_ALWAYS_INLINE_ATTR 1
	#endif
#endif

#ifdef __has_builtin // since GCC 10.0.0 and clang 1.0.0 (which thus requires no version check)
	#if __has_builtin(__builtin_expect)
		#define JXSML__HAS_BUILTIN_EXPECT 1
	#endif
	#if __has_builtin(__builtin_add_overflow)
		#define JXSML__HAS_BUILTIN_ADD_OVERFLOW 1
	#endif
	#if __has_builtin(__builtin_sub_overflow)
		#define JXSML__HAS_BUILTIN_SUB_OVERFLOW 1
	#endif
	#if __has_builtin(__builtin_mul_overflow)
		#define JXSML__HAS_BUILTIN_MUL_OVERFLOW 1
	#endif
	#if __has_builtin(__builtin_unreachable)
		#define JXSML__HAS_BUILTIN_UNREACHABLE 1
	#endif
	#if __has_builtin(__builtin_assume_aligned)
		#define JXSML__HAS_BUILTIN_ASSUME_ALIGNED 1
	#endif
#endif
...
// clang (among many others) fakes GCC version by default, but we handle clang separately
...
#ifdef __clang__
	#define JXSML__CLANG_VER (__clang_major__ * 0x10000 + __clang_minor__ * 0x100 + __clang_patchlevel__)
#else
	...
	#if JXSML__HAS_ALWAYS_INLINE_ATTR || JXSML__GCC_VER >= 0x30100 || JXSML__CLANG_VER >= 0x10000
...
#ifndef JXSML_EXPECT
	#if JXSML__HAS_BUILTIN_EXPECT || JXSML__GCC_VER >= 0x30000
...
#if !defined JXSML_ADD_OVERFLOW && (JXSML__HAS_BUILTIN_ADD_OVERFLOW || JXSML__GCC_VER >= 0x50000)
	#define JXSML_ADD_OVERFLOW(a, b, res) __builtin_add_overflow(a, b, res)
#endif
#if !defined JXSML_SUB_OVERFLOW && (JXSML__HAS_BUILTIN_SUB_OVERFLOW || JXSML__GCC_VER >= 0x50000)
	#define JXSML_SUB_OVERFLOW(a, b, res) __builtin_sub_overflow(a, b, res)
#endif
#if !defined JXSML_MUL_OVERFLOW && (JXSML__HAS_BUILTIN_MUL_OVERFLOW || JXSML__GCC_VER >= 0x50000)
	#define JXSML_MUL_OVERFLOW(a, b, res) __builtin_mul_overflow(a, b, res)
#endif

#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_PLATFORM && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// public API

#if defined JXSML__PART_API && JXSML__RECURSING < 0

// an internal error type. non-zero indicates a different error condition.
// user callbacks can also emit error codes, which should not exceed `JXSML_MIN_RESERVED_ERR`.
// it can be interpreted as a four-letter code, but such encoding is not guaranteed.
typedef uint32_t jxsml_err;
#define JXSML_MIN_RESERVED_ERR (jxsml_err) (1 << 24) // anything below this can be used freely

#endif // defined JXSML__PART_API && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// state

#if defined JXSML__PART_STATE && JXSML__RECURSING < 0

// a common context ("state") for all internal functions.
typedef struct {
	jxsml_err err; // first error code encountered, or 0
	int saved_errno;
	...
	struct jxsml__source *source;

	// different subsystems make use of additional contexts, all accessible from here.
	struct jxsml__container_st *container;
	struct jxsml__image_st *image;
	struct jxsml__frame_st *frame;
	//struct jxsml__lfgroup_st *lfgroup;
} jxsml__st;

#endif // defined JXSML__PART_STATE && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// error handling

#if defined JXSML__PART_ERROR && JXSML__RECURSING < 0
...
#define JXSML__SHOULD_OR(cond, s, stmt) do { \
		if (JXSML_UNLIKELY(st->err)) { stmt; } \
		else if (JXSML_UNLIKELY(!(cond))) { jxsml__set_error(st, JXSML__4(s)); stmt; } \
	} while (0)
#define JXSML__ON_ERROR error // goto label
#define JXSML__SHOULD(cond, s) JXSML__SHOULD_OR(cond, s, goto JXSML__ON_ERROR)
#define JXSML__RAISE(s) do { jxsml__set_error(st, JXSML__4(s)); goto JXSML__ON_ERROR; } while (0)
#define JXSML__RAISE_DELAYED() do { if (JXSML_UNLIKELY(st->err)) goto JXSML__ON_ERROR; } while (0)
#define JXSML__TRY(expr) do { if (JXSML_UNLIKELY(expr)) goto JXSML__ON_ERROR; } while (0)
...
#define JXSML__INBOUNDS(ptr, start, size) ((uintptr_t) (ptr) - (uintptr_t) (start) <= (uintptr_t) (size))
#ifdef JXSML_DEBUG
	#define JXSML__ASSERT(cond) assert(cond)
	#define JXSML__UNREACHABLE() JXSML__ASSERT(0)
#elif JXSML__HAS_BUILTIN_UNREACHABLE || JXSML__GCC_VER >= 0x40500
	#define JXSML__ASSERT(cond) (JXSML_UNLIKELY(!(cond)) ? __builtin_unreachable() : (void) 0)
	#define JXSML__UNREACHABLE() __builtin_unreachable()
#else
	#define JXSML__ASSERT(cond) ((void) 0)
	#define JXSML__UNREACHABLE() ((void) 0) // TODO also check for MSVC __assume
#endif
...
JXSML_STATIC jxsml_err jxsml__set_error(jxsml__st *st, jxsml_err err) {
	if (err != JXSML__4("shrt")) st->cannot_retry = 1;
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_ERROR && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// utility

#if defined JXSML__PART_UTILITY && JXSML__RECURSING < 0
...
// `jxsml__(foo, X)` is `jxsml__foo` followed by a macro `JXSML__V` expanded;
...
JXSML_ALWAYS_INLINE int32_t jxsml__unpack_signed(int32_t x);
JXSML_ALWAYS_INLINE int32_t jxsml__ceil_div32(int32_t x, int32_t y);
...
#endif // defined JXSML__PART_UTILITY && JXSML__RECURSING < 0
#if JXSML__RECURSING == 100
	#define jxsml__intN_t JXSML__CONCAT3(int, JXSML__N, _t)
	#define jxsml__uintN_t JXSML__CONCAT3(uint, JXSML__N, _t)
	#define JXSML__INTN_MAX JXSML__CONCAT3(INT, JXSML__N, _MAX)
	#define JXSML__INTN_MIN JXSML__CONCAT3(INT, JXSML__N, _MIN)
// ----------------------------------------
...
JXSML_ALWAYS_INLINE jxsml__intN_t jxsml__(add,N)(jxsml__st *st, jxsml__intN_t x, jxsml__intN_t y);
JXSML_ALWAYS_INLINE jxsml__intN_t jxsml__(sub,N)(jxsml__st *st, jxsml__intN_t x, jxsml__intN_t y);
JXSML_ALWAYS_INLINE jxsml__intN_t jxsml__(mul,N)(jxsml__st *st, jxsml__intN_t x, jxsml__intN_t y);
...
JXSML_ALWAYS_INLINE jxsml__intN_t jxsml__(add,N)(jxsml__st *st, jxsml__intN_t x, jxsml__intN_t y) {
#ifdef JXSML_ADD_OVERFLOW
	jxsml__intN_t res;
	if (JXSML_UNLIKELY(JXSML_ADD_OVERFLOW(x, y, &res))) JXSML__ERR("over");
	return res;
#else
	if (JXSML_UNLIKELY((x > 0 && y > JXSML__INTN_MAX - x) || (x < 0 && y < JXSML__INTN_MIN - x))) {
		return JXSML__ERR("over"), x;
	} else {
		return (jxsml__intN_t) (x + y);
	}
#endif
}

JXSML_ALWAYS_INLINE jxsml__intN_t jxsml__(sub,N)(jxsml__st *st, jxsml__intN_t x, jxsml__intN_t y) {
#ifdef JXSML_SUB_OVERFLOW
	jxsml__intN_t res;
	if (JXSML_UNLIKELY(JXSML_SUB_OVERFLOW(x, y, &res))) JXSML__ERR("over");
	return res;
#else
	if (JXSML_UNLIKELY((y < 0 && x > JXSML__INTN_MAX + y) || (y > 0 && x < JXSML__INTN_MIN + y))) {
		return JXSML__ERR("over"), x;
	} else {
		return (jxsml__intN_t) (x - y);
	}
...
JXSML_ALWAYS_INLINE jxsml__intN_t jxsml__(mul,N)(jxsml__st *st, jxsml__intN_t x, jxsml__intN_t y) {
#ifdef JXSML_MUL_OVERFLOW
	jxsml__intN_t res;
	if (JXSML_UNLIKELY(JXSML_MUL_OVERFLOW(x, y, &res))) JXSML__ERR("over");
	return res;
#else
	if (JXSML_UNLIKELY(
		(x == -1 && y == JXSML__INTN_MIN) || (y == -1 && x == JXSML__INTN_MIN) ||
		(x != 0 && (y > JXSML__INTN_MAX / x || y < JXSML__INT_MIN / x))
	)) {
		return JXSML__ERR("over"), x;
	} else {
		return (jxsml__intN_t) (x * y);
...
#undef JXSML__UINTN_MAX
	...
	JXSML_ALWAYS_INLINE int jxsml__(ceil_lg,N)(jxsml__uintN_t x) {
		return x > 1 ? JXSML__N - JXSML__CLZN(x - 1) : 0;
	}
	#endif
...
// end of recursion
	...
	#undef JXSML__INTN_MAX
	#undef JXSML__INTN_MIN
	#undef JXSML__N
...
#if defined JXSML__PART_ALIGNED_PTRS && JXSML__RECURSING < 0

// TODO simplify this if possible

#ifndef JXSML_ASSUME_ALIGNED
	#if JXSML__HAS_BUILTIN_ASSUME_ALIGNED || JXSML__GCC_VER >= 0x40700
...
JXSML_ALWAYS_INLINE void jxsml__free_aligned_(void *ptr, size_t align);

#define JXSML__ALIGNED_(ptr, align) \
	(JXSML__ASSERT((intptr_t) (ptr) % (align) == 0), JXSML_ASSUME_ALIGNED(ptr, align))
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_ALIGNED_PTRS && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// two-dimensional view

#if defined JXSML__PART_VIEW && JXSML__RECURSING < 0

typedef struct { int32_t logw, logh; float *JXSML_RESTRICT ptr; } jxsml__view_f32;

JXSML_ALWAYS_INLINE jxsml__view_f32 jxsml__make_view_f32(int32_t logw, int32_t logh, float *JXSML_RESTRICT ptr) {
	jxsml__view_f32 ret = { logw, logh, ptr };
	return ret;
}

JXSML_ALWAYS_INLINE void jxsml__adapt_view_f32(jxsml__view_f32 *outv, int32_t logw, int32_t logh) {
	JXSML__ASSERT(outv->logw + outv->logh >= logw + logh);
	outv->logw = logw;
	outv->logh = logh;
}

JXSML_ALWAYS_INLINE void jxsml__reshape_view_f32(jxsml__view_f32 *outv, int32_t logw, int32_t logh) {
	JXSML__ASSERT(outv->logw + outv->logh == logw + logh);
	outv->logw = logw;
	outv->logh = logh;
}

JXSML_ALWAYS_INLINE void jxsml__copy_view_f32(jxsml__view_f32 *outv, const jxsml__view_f32 inv) {
	int32_t x, y;
	float *outptr = outv->ptr;
	jxsml__adapt_view_f32(outv, inv.logw, inv.logh);
	for (y = 0; y < (1 << inv.logh); ++y) {
		for (x = 0; x < (1 << inv.logw); ++x) outptr[y << inv.logw | x] = inv.ptr[y << inv.logw | x];
	}
}

JXSML_ALWAYS_INLINE void jxsml__transpose_view_f32(jxsml__view_f32 *outv, const jxsml__view_f32 inv) {
	int32_t x, y;
	float *outptr = outv->ptr;
	jxsml__adapt_view_f32(outv, inv.logh, inv.logw);
	for (y = 0; y < (1 << inv.logh); ++y) {
		for (x = 0; x < (1 << inv.logw); ++x) outptr[x << inv.logh | y] = inv.ptr[y << inv.logw | x];
	}
}

// shuffles columns 01234567 into 02461357 and so on
JXSML_ALWAYS_INLINE void jxsml__oddeven_columns_to_halves_f32(jxsml__view_f32 *outv, const jxsml__view_f32 inv) {
	int32_t x, y;
	float *outptr = outv->ptr;
	JXSML__ASSERT(inv.logw > 0);
	jxsml__adapt_view_f32(outv, inv.logw, inv.logh);
	for (y = 0; y < (1 << inv.logh); ++y) {
...
JXSML_ALWAYS_INLINE void jxsml__oddeven_rows_to_halves_f32(jxsml__view_f32 *outv, const jxsml__view_f32 inv) {
	int32_t x, y;
	float *outptr = outv->ptr;
	JXSML__ASSERT(inv.logh > 0);
	jxsml__adapt_view_f32(outv, inv.logw, inv.logh);
	for (y = 0; y < (1 << inv.logh); ++y) {
...
#define JXSML__AT(view, x, y) \
	(JXSML__ASSERT(0 <= (x) && (x) < (1 << (view).logw) && 0 <= (y) && (y) < (1 << (view).logh)), \
	 (view).ptr + ((y) << (view).logw | (x)))

#define JXSML__VIEW_FOREACH(view, y, x, v) \
	for (y = 0; y < (1 << (view).logh); ++y) \
		for (x = 0; x < (1 << (view).logw) && (v = (view).ptr + (y << (view).logw | x), 1); ++x)

#ifdef JXSML_DEBUG
JXSML_STATIC void jxsml__print_view_f32(jxsml__view_f32 v, const char *name, const char *file, int32_t line) {
	int32_t x, y;
	printf(".--- %s:%d: %s (w=%d h=%d @%p)", file, line, name, 1 << v.logw, 1 << v.logh, v.ptr);
	for (y = 0; y < (1 << v.logh); ++y) {
		printf("\n|");
		for (x = 0; x < (1 << v.logw); ++x) printf(" %f", *JXSML__AT(v, x, y));
	}
	printf("\n'--- %s:%d\n", file, line);
}
#define jxsml__print_view_f32(v) jxsml__print_view_f32(v, #v, __FILE__, __LINE__)
#endif

#endif // defined JXSML__PART_VIEW && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// plane

#if defined JXSML__PART_PLANE && JXSML__RECURSING < 0

/*
enum {
	JXSML__PLANE_U8 = 'u' << 8 | 8,
	JXSML__PLANE_U16 = 'u' << 8 | 16,
	JXSML__PLANE_I16 = 'i' << 8 | 16,
	JXSML__PLANE_I32 = 'i' << 8 | 32,
	JXSML__PLANE_F32 = 'f' << 8 | 32,
};

typedef struct {
	int32_t width, height;
	int32_t stride; // the number of *bytes* between each row
	int16_t type; // 0 means uninitialized
	...
	union {
		...
		int32_t *i32;
		float *f32;
	} pixels; // aligned like PIXELS
} jxsml__plane;

JXSML_STATIC void *jxsml__init_plane_common(
	jxsml__st *st, int type, int32_t width, int32_t height, size_t pixelsize, jxsml__plane *out
) {
	void *pixels;
	int32_t stride = jxsml__mul32(st,
		jxsml__ceil_div32(
			jxsml__mul32(st, width, (int32_t) pixelsize),
			JXSML__ALIGNMENT_PIXELS),
		JXSML__ALIGNMENT_PIXELS);

	JXSML__ASSERT(width > 0 && height > 0);
	JXSML__SHOULD(stride <= SIZE_MAX / (uint32_t) width, "over");
	JXSML__SHOULD(pixels = JXSML__ALLOC_ALIGNED(PIXELS, stride * width), "!mem");
	out->stride = stride;
	out->width = width;
	out->height = height;
	out->type = (int16_t) type;
	out->vshift = out->hshift = 0;
JXSML__ON_ERROR:
	return pixels;
}

JXSML_INLINE jxsml_err jxsml__init_plane_u8(jxsml__st *st, int32_t width, int32_t height, jxsml__plane *out) {
	out->pixels.u8 = jxsml__init_plane_common(st, JXSML__PLANE_U8, width, height, sizeof(uint8_t), out);
	return st->err;
}

JXSML_INLINE jxsml_err jxsml__init_plane_u16(jxsml__st *st, int32_t width, int32_t height, jxsml__plane *out) {
	out->pixels.u16 = jxsml__init_plane_common(st, JXSML__PLANE_U16, width, height, sizeof(uint16_t), out);
	return st->err;
}

JXSML_INLINE jxsml_err jxsml__init_plane_i16(jxsml__st *st, int32_t width, int32_t height, jxsml__plane *out) {
	out->pixels.i16 = jxsml__init_plane_common(st, JXSML__PLANE_I16, width, height, sizeof(int16_t), out);
	return st->err;
}

JXSML_INLINE jxsml_err jxsml__init_plane_i32(jxsml__st *st, int32_t width, int32_t height, jxsml__plane *out) {
	out->pixels.i32 = jxsml__init_plane_common(st, JXSML__PLANE_I32, width, height, sizeof(int32_t), out);
	return st->err;
}

JXSML_INLINE jxsml_err jxsml__init_plane_f32(jxsml__st *st, int32_t width, int32_t height, jxsml__plane *out) {
	out->pixels.f32 = jxsml__init_plane_common(st, JXSML__PLANE_F32, width, height, sizeof(float), out);
	return st->err;
}

JXSML_STATIC void jxsml__free_plane(jxsml__plane *plane) {
	switch (plane->type) {
	case JXSML__PLANE_U8: JXSML__FREE_ALIGNED(PIXELS, plane->pixels.u8); break;
	case JXSML__PLANE_U16: JXSML__FREE_ALIGNED(PIXELS, plane->pixels.u16); break;
	case JXSML__PLANE_I16: JXSML__FREE_ALIGNED(PIXELS, plane->pixels.i16); break;
	case JXSML__PLANE_I32: JXSML__FREE_ALIGNED(PIXELS, plane->pixels.i32); break;
	case JXSML__PLANE_F32: JXSML__FREE_ALIGNED(PIXELS, plane->pixels.f32); break;
	}
	plane->width = plane->height = plane->stride = 0;
	plane->type = 0;
	plane->vshift = plane->hshift = 0;
	plane->pixels.u8 = NULL; 
}
*/

#endif // defined JXSML__PART_PLANE && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// input source

#if defined JXSML__PART_SOURCE && JXSML__RECURSING < 0

typedef void (*jxsml_memory_free_func)(void *data);

typedef jxsml_err (*jxsml_source_read_func)(uint8_t *buf, size_t maxsize, size_t *size, void *data);
//typedef jxsml_err (*jxsml_source_jump_func)(size_t offset, void *data, void **outdata);
typedef void (*jxsml_source_free_func)(void *data);

typedef struct jxsml__source {
	...
	uint8_t *buf;
	jxsml_memory_free_func buf_free_func; // set if the buffer is owned, used to deallocate `buf`
	...
	jxsml_source_read_func read_func;
	//jxsml_source_jump_func jump_func;
	jxsml_source_free_func free_func;
	void *data;
} jxsml__source;

JXSML_STATIC void jxsml__init_memory_source(uint8_t *, size_t, jxsml_memory_free_func, jxsml__source *);
JXSML_STATIC jxsml_err jxsml__init_file_source(jxsml__st *, const char *, size_t, jxsml__source *);
JXSML_STATIC jxsml_err jxsml__refill_backing_buffer(jxsml__st *);
JXSML_STATIC void jxsml__free_source(jxsml__source *);

#ifdef JXSML_IMPLEMENTATION

JXSML_STATIC void jxsml__init_memory_source(
	uint8_t *buf, size_t size, jxsml_memory_free_func freefunc, jxsml__source *s
) {
...
JXSML_STATIC jxsml_err jxsml__file_source_read(uint8_t *buf, size_t maxsize, size_t *size, void *data) {
	FILE *fp = data;
	...
	if (read > 0) {
		...
		*size = 0;
		...
		return JXSML__4("read");
	}
}

JXSML_STATIC void jxsml__file_source_free(void *data) {
	FILE *fp = data;
	fclose(fp);
}

JXSML_STATIC jxsml_err jxsml__init_file_source(jxsml__st *st, const char *path, size_t bufsize, jxsml__source *s) {
	...
	JXSML__SHOULD(s->checkpoint = s->buf = malloc(bufsize), "!mem");
	s->buf_free_func = free;
	s->size = 0;
	s->capacity = bufsize;
	s->bytes_before_buf = 0;
	s->read_func = jxsml__file_source_read;
	s->free_func = jxsml__file_source_free;
	s->data = fp;
	return 0;
...
JXSML_STATIC jxsml_err jxsml__refill_backing_buffer(jxsml__st *st) {
	jxsml__source *source = st->source;
	size_t committed_size, ptr_pos;

	if (!source->read_func) {
		st->cannot_retry = 1;
		return JXSML__ERR("shrt");
	}
	...
	JXSML__ASSERT(JXSML__INBOUNDS(st->ptr, source->buf, source->size));
	JXSML__ASSERT(JXSML__INBOUNDS(source->checkpoint, source->buf, source->size));
	JXSML__ASSERT(source->checkpoint <= st->ptr);
	...
	if (source->size == source->capacity) {
		...
		uint8_t *newbuf;
		JXSML__SHOULD(newcap < source->capacity, "!mem");
		newbuf = realloc(source->buf, newcap);
		JXSML__SHOULD(newbuf, "!mem");
...
JXSML_STATIC void jxsml__free_source(jxsml__source *s) {
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_SOURCE && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// container

#if defined JXSML__PART_CONTAINER && JXSML__RECURSING < 0

typedef struct jxsml__container_st {
	...
	enum {
		...
		JXSML__CONTAINER_CONFIRMED = 1 << 0,

		// currently seen box types, as they have a cardinality and positional requirement
		JXSML__SEEN_JXLL = 1 << 1, // at most once, before jxlc/jxlp
		JXSML__SEEN_JXLI = 1 << 2, // at most once
		JXSML__SEEN_JXLC = 1 << 3, // precludes jxlp, at most once
		JXSML__SEEN_JXLP = 1 << 4, // precludes jxlc

		// if set, no more jxlc/jxlp boxes are allowed
		JXSML__NO_MORE_CODESTREAM_BOX = 1 << 5,
	} flags;

	// only applicable after JXSML__CONTAINER_CONFIRMED is set
	enum {
		JXSML__BOX_HEADER = 0,
		JXSML__CODESTREAM_BOX = 1, // box contents of jxlc/jxlp
		JXSML__NON_CODESTREAM_BOX = 2, // box contents of everything else
	} next;
} jxsml__container_st;

JXSML_STATIC jxsml_err jxsml__container(jxsml__st *st);

#ifdef JXSML_IMPLEMENTATION

JXSML_STATIC jxsml_err jxsml__container_u32(jxsml__st *st, uint32_t *v) {
	JXSML__SHOULD(st->remaining >= 4, "shrt");
...
JXSML_STATIC jxsml_err jxsml__box_header(jxsml__st *st, uint32_t *type, int *brotli, uint64_t *size) {
	uint32_t size32;

	JXSML__TRY(jxsml__container_u32(st, &size32));
	JXSML__TRY(jxsml__container_u32(st, type));
	if (size32 == 0) {
		*size = UINT64_MAX;
	} else if (size32 == 1) {
		...
		*size = ((uint64_t) sizehi << 32) | (uint64_t) sizelo;
		JXSML__SHOULD(*size >= 16, "boxx");
		*size -= 16;
		JXSML__SHOULD(st->remaining >= *size, "shrt");
	} else {
	...
	*brotli = (*type == 0x62726f62 /*brob*/);
	if (*brotli) {
		JXSML__SHOULD(*size > 4, "brot"); // Brotli stream is never empty so 4 is also out
		JXSML__TRY(jxsml__container_u32(st, type));
		JXSML__SHOULD(*type != 0x62726f62 /*brob*/ && (*type >> 8) != 0x6a786c /*jxl*/, "brot");
...
JXSML_STATIC jxsml_err jxsml__container(jxsml__st *st) {
	...
	jxsml__source *source = st->source;
	jxsml__container_st *container = st->container;

	// ensure that we came here because the logical buffer has been exhausted...
	JXSML__ASSERT(st->remaining == 0);
	...
	if (!(container->flags & JXSML__CONTAINER_CONFIRMED)) {
		// format identification, which should work correctly even with a partial input!
		JXSML__SHOULD(st->remaining > 0, "shrt");
		...
		if (st->remaining < sizeof(JXL_BOX)) {
			...
			JXSML__SHOULD(memcmp(st->ptr, JXL_BOX, st->remaining) == 0, "!jxl");
			JXSML__RAISE("shrt");
		}

		JXSML__SHOULD(memcmp(st->ptr, JXL_BOX, sizeof(JXL_BOX)) == 0, "!jxl");
		st->ptr += sizeof(JXL_BOX);
		st->remaining -= sizeof(JXL_BOX);
		JXSML__SHOULD(st->remaining >= sizeof(FTYP_BOX), "shrt");
		JXSML__SHOULD(memcmp(st->ptr, FTYP_BOX, sizeof(FTYP_BOX)) == 0, "ftyp");
		st->ptr += sizeof(FTYP_BOX);
		st->remaining -= sizeof(FTYP_BOX);

		source->checkpoint = st->ptr;
		container->flags |= JXSML__CONTAINER_CONFIRMED;
	}

	while (st->remaining > 0) {
		if (container->next == JXSML__BOX_HEADER) {
			uint32_t type;
			int brotli;
			uint64_t size;
			unsigned new_flags = 0;

			JXSML__TRY(jxsml__box_header(st, &type, &brotli, &size));

			// TODO the ordering rule for jxll/jxli may change in the future version of 18181-2
			switch (type) {
			case 0x6a786c6c: // jxll: codestream level
				JXSML__SHOULD(!(container->flags & JXSML__SEEN_JXLL), "box?");
				new_flags = JXSML__SEEN_JXLL;
				break;

			case 0x6a786c69: // jxli: frame index
				JXSML__SHOULD(!(container->flags & JXSML__SEEN_JXLI), "box?");
				new_flags = JXSML__SEEN_JXLI;
				break;

			case 0x6a786c63: // jxlc: single codestream
				JXSML__ASSERT(!brotli);
				JXSML__SHOULD(!(container->flags & JXSML__NO_MORE_CODESTREAM_BOX), "box?");
				JXSML__SHOULD(!(container->flags & (JXSML__SEEN_JXLP | JXSML__SEEN_JXLC)), "box?");
				new_flags = JXSML__SEEN_JXLC;
				break;

			case 0x6a786c70: // jxlp: partial codestreams
				JXSML__ASSERT(!brotli);
				JXSML__SHOULD(!(container->flags & JXSML__NO_MORE_CODESTREAM_BOX), "box?");
				JXSML__SHOULD(!(container->flags & JXSML__SEEN_JXLC), "box?");
				new_flags = JXSML__SEEN_JXLP;
				JXSML__SHOULD(size >= 4, "jxlp");
				JXSML__SHOULD(st->remaining >= 4, "shrt");
				// TODO the partial codestream index is ignored right now
				if (!(st->ptr[0] >> 7)) new_flags |= JXSML__NO_MORE_CODESTREAM_BOX;
				st->ptr += 4;
				st->remaining -= 4;
			...
			if (new_flags & (JXSML__SEEN_JXLP | JXSML__SEEN_JXLC)) {
				container->next = JXSML__CODESTREAM_BOX;
			} else {
				container->next = JXSML__NON_CODESTREAM_BOX;
			}
			container->box_remaining = size;
			source->checkpoint = st->ptr;
		}

		JXSML__ASSERT(container->next != JXSML__BOX_HEADER);
		if (container->next == JXSML__CODESTREAM_BOX) {
			...
			if (container->box_remaining < SIZE_MAX) {
				...
				if (container->box_remaining == 0) container->next = JXSML__BOX_HEADER;
			}
			return 0;
		} else {
			...
			if (container->box_remaining < SIZE_MAX) {
				...
				JXSML__SHOULD(container->box_remaining == 0, "shrt");
				container->next = JXSML__BOX_HEADER;
			}
		}
	}

	JXSML__SHOULD(container->flags & JXSML__NO_MORE_CODESTREAM_BOX, "shrt");

JXSML__ON_ERROR:
	return st->err;
}

// TODO jxsml__finish_container
...
#if defined JXSML__PART_BITS && JXSML__RECURSING < 0

JXSML_STATIC size_t jxsml__bits_read(const jxsml__st *st);
...
#define jxsml__refill(st, n) (JXSML_UNLIKELY(st->nbits < (n)) ? jxsml__always_refill(st, n) : st->err)
...
JXSML_STATIC uint64_t jxsml__u64(jxsml__st *st);
JXSML_INLINE int32_t jxsml__enum(jxsml__st *st);
JXSML_INLINE float jxsml__f16(jxsml__st *st);
JXSML_STATIC uint64_t jxsml__varint(jxsml__st *st);
JXSML_INLINE int32_t jxsml__u8(jxsml__st *st);
...
JXSML_STATIC jxsml_err jxsml__always_refill(jxsml__st *st, int32_t n) {
	...
	do {
		...
		if (st->remaining < consumed) {
			...
			if (st->nbits < n) {
				JXSML__SHOULD(st->container, "shrt");
				JXSML__SHOULD(!(st->container->flags & JXSML__NO_MORE_CODESTREAM_BOX), "shrt");
				JXSML__TRY(jxsml__container(st));
				continue; // now we have possibly more bits to refill, try again
			}
		} else {
			st->remaining -= consumed;
			JXSML__ASSERT(st->nbits <= 24);
			do {
				st->bits |= (uint32_t) *st->ptr++ << st->nbits;
				st->nbits += 8;
			} while (st->nbits <= 24);
...
JXSML_STATIC jxsml_err jxsml__skip(jxsml__st *st, uint64_t n) {
	...
	if (st->remaining < bytes) return JXSML__ERR("shrt");
	st->remaining -= bytes;
	st->ptr += bytes;
	n &= 7;
...
JXSML_INLINE int32_t jxsml__u32(
	...
	int32_t sel;
	JXSML__ASSERT(0 <= n0 && n0 <= 30 && o0 <= 0x7fffffff - (1 << n0));
	JXSML__ASSERT(0 <= n1 && n1 <= 30 && o1 <= 0x7fffffff - (1 << n1));
	JXSML__ASSERT(0 <= n2 && n2 <= 30 && o2 <= 0x7fffffff - (1 << n2));
	JXSML__ASSERT(0 <= n3 && n3 <= 30 && o3 <= 0x7fffffff - (1 << n3));
	sel = jxsml__u(st, 2);
...
JXSML_STATIC uint64_t jxsml__varint(jxsml__st *st) { // ICC only
	...
	int32_t shift = 0;
	do {
		if (st->remaining == 0) return JXSML__ERR("shrt"), (uint64_t) 0;
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_BITS && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// prefix code

#if defined JXSML__PART_PREFIX_CODE && JXSML__RECURSING < 0
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_PREFIX_CODE && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// hybrid integer encoding

#if defined JXSML__PART_HYBRID_INT && JXSML__RECURSING < 0
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_HYBRID_INT && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// rANS alias table

#if defined JXSML__PART_ANS_CODE && JXSML__RECURSING < 0

enum {
	JXSML__DIST_BITS = 12,
	JXSML__ANS_INIT_STATE = 0x130000
};
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_ANS_CODE && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// entropy code

#if defined JXSML__PART_ENTROPY_CODE && JXSML__RECURSING < 0
...
JXSML_STATIC int32_t jxsml__entropy_code_cluster(
	jxsml__st *st, int use_prefix_code, int32_t log_alpha_size,
	jxsml__code_cluster_t *cluster, uint32_t *ans_state
);
JXSML_STATIC int32_t jxsml__code(jxsml__st *st, int32_t ctx, int32_t dist_mult, jxsml__code_t *code);
...
continue_lz77:
	...
	JXSML__ASSERT(ctx < spec->num_dist);
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_ENTROPY_CODE && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// image header & metadata

#if defined JXSML__PART_IMAGE_HEADER && JXSML__RECURSING < 0

enum {
	JXSML__CHROMA_WHITE = 0, JXSML__CHROMA_RED = 1,
	JXSML__CHROMA_GREEN = 2, JXSML__CHROMA_BLUE = 3,
};

typedef struct {
	enum {
		JXSML__EC_ALPHA = 0, JXSML__EC_DEPTH = 1, JXSML__EC_SPOT_COLOUR = 2,
		JXSML__EC_SELECTION_MASK = 3, JXSML__EC_BLACK = 4, JXSML__EC_CFA = 5,
		JXSML__EC_THERMAL = 6, JXSML__EC_NON_OPTIONAL = 15, JXSML__EC_OPTIONAL = 16,
	} type;
...
typedef struct jxsml__image_st {
	int32_t width, height;
	enum {
		JXSML__ORIENT_TL = 1, JXSML__ORIENT_TR = 2, JXSML__ORIENT_BR = 3, JXSML__ORIENT_BL = 4,
		JXSML__ORIENT_LT = 5, JXSML__ORIENT_RT = 6, JXSML__ORIENT_RB = 7, JXSML__ORIENT_LB = 8,
	} orientation;
	...
	char *icc;
	size_t iccsize;
	enum { JXSML__CS_CHROMA = 'c', JXSML__CS_GREY = 'g', JXSML__CS_XYB = 'x' } cspace;
	float cpoints[4 /*JXSML__CHROMA_xxx*/][2 /*x=0, y=1*/]; // only for JXSML__CS_CHROMA
	enum {
		JXSML__TF_709 = -1, JXSML__TF_UNKNOWN = -2, JXSML__TF_LINEAR = -8, JXSML__TF_SRGB = -13,
		JXSML__TF_PQ = -16, JXSML__TF_DCI = -17, JXSML__TF_HLG = -18,
		JXSML__GAMMA_MAX = 10000000,
	} gamma_or_tf; // gamma if > 0, transfer function if <= 0
	enum { JXSML__INTENT_PERC = 0, JXSML__INTENT_REL = 1, JXSML__INTENT_SAT = 2, JXSML__INTENT_ABS = 3 } render_intent;
...
JXSML_STATIC jxsml_err jxsml__customxy(jxsml__st *st, float xy[2]) {
	xy[0] = (float)jxsml__unpack_signed(jxsml__u32(st, 0, 19, 0x80000, 19, 0x100000, 20, 0x200000, 21)) / 100000.0f;
	xy[1] = (float)jxsml__unpack_signed(jxsml__u32(st, 0, 19, 0x80000, 19, 0x100000, 20, 0x200000, 21)) / 100000.0f;
	return st->err;
}
...
JXSML_STATIC jxsml_err jxsml__image_metadata(jxsml__st *st) {
	...
	jxsml__image_st *im = st->image;
	int32_t i, j;

	im->orientation = JXSML__ORIENT_TL;
	im->intr_width = 0;
	im->intr_height = 0;
	im->bpp = 8;
	im->exp_bits = 0;
	...
	im->icc = NULL;
	im->iccsize = 0;
	im->cspace = JXSML__CS_CHROMA;
	memcpy(im->cpoints, SRGB_CHROMA, sizeof SRGB_CHROMA);
	im->gamma_or_tf = JXSML__TF_SRGB;
	im->render_intent = JXSML__INTENT_REL;
	...
	if (!jxsml__u(st, 1)) { // !all_default
		...
		if (extra_fields) {
			im->orientation = jxsml__u(st, 3) + 1;
			if (jxsml__u(st, 1)) { // have_intr_size
				JXSML__TRY(jxsml__size_header(st, &im->intr_width, &im->intr_height));
			}
			if (jxsml__u(st, 1)) { // have_preview
				JXSML__RAISE("TODO: preview");
			}
			if (jxsml__u(st, 1)) { // have_animation
				im->anim_tps_num = jxsml__u32(st, 100, 0, 1000, 0, 1, 10, 1, 30);
				im->anim_tps_denom = jxsml__u32(st, 1, 0, 1001, 0, 1, 8, 1, 10);
				im->anim_nloops = jxsml__u32(st, 0, 0, 0, 3, 0, 16, 0, 32);
				im->anim_have_timecodes = jxsml__u(st, 1);
			}
		}
		JXSML__TRY(jxsml__bit_depth(st, &im->bpp, &im->exp_bits));
		im->modular_16bit_buffers = jxsml__u(st, 1);
		im->num_extra_channels = jxsml__u32(st, 0, 0, 1, 0, 2, 4, 1, 12);
		JXSML__SHOULD(im->ec_info = calloc((size_t) im->num_extra_channels, sizeof(jxsml__ec_info)), "!mem");
		for (i = 0; i < im->num_extra_channels; ++i) {
			jxsml__ec_info *ec = &im->ec_info[i];
			if (jxsml__u(st, 1)) { // d_alpha
				ec->type = JXSML__EC_ALPHA;
				ec->bpp = 8;
				...
				switch (ec->type) {
				case JXSML__EC_ALPHA:
					ec->data.alpha_associated = jxsml__u(st, 1);
					break;
				case JXSML__EC_SPOT_COLOUR:
				...
				case JXSML__EC_CFA:
					...
					break;
				case JXSML__EC_DEPTH: case JXSML__EC_SELECTION_MASK: case JXSML__EC_BLACK:
				case JXSML__EC_THERMAL: case JXSML__EC_NON_OPTIONAL: case JXSML__EC_OPTIONAL:
					break;
		...
		if (!jxsml__u(st, 1)) { // ColourEncoding.all_default
			...
			im->want_icc = jxsml__u(st, 1);
			cspace = jxsml__enum(st);
			switch (cspace) {
			case CS_RGB: case CS_UNKNOWN: im->cspace = JXSML__CS_CHROMA; break;
			case CS_GREY: im->cspace = JXSML__CS_GREY; break;
			case CS_XYB: im->cspace = JXSML__CS_XYB; break;
			...
			if (!im->want_icc) {
				if (cspace != CS_XYB) {
					...
					case WP_CUSTOM: JXSML__TRY(jxsml__customxy(st, im->cpoints[JXSML__CHROMA_WHITE])); break;
					case WP_E: memcpy(im->cpoints + JXSML__CHROMA_WHITE, E, sizeof E); break;
					case WP_DCI: memcpy(im->cpoints + JXSML__CHROMA_WHITE, DCI, sizeof DCI); break;
					...
					if (cspace != CS_GREY) {
						...
						case PR_CUSTOM:
							JXSML__TRY(jxsml__customxy(st, im->cpoints[JXSML__CHROMA_RED]));
							JXSML__TRY(jxsml__customxy(st, im->cpoints[JXSML__CHROMA_GREEN]));
							JXSML__TRY(jxsml__customxy(st, im->cpoints[JXSML__CHROMA_BLUE]));
							break;
						case PR_2100: memcpy(im->cpoints + JXSML__CHROMA_RED, BT2100, sizeof BT2100); break;
						case PR_P3: memcpy(im->cpoints + JXSML__CHROMA_RED, P3, sizeof P3); break;
						default: JXSML__RAISE("prm?");
						}
					}
				}
				if (jxsml__u(st, 1)) { // have_gamma
					im->gamma_or_tf = jxsml__u(st, 24);
					JXSML__SHOULD(im->gamma_or_tf > 0 && im->gamma_or_tf <= JXSML__GAMMA_MAX, "gama");
					if (cspace == CS_XYB) JXSML__SHOULD(im->gamma_or_tf == 3333333, "gama");
				} else {
					im->gamma_or_tf = -jxsml__enum(st);
					JXSML__SHOULD((
						1 << -JXSML__TF_709 | 1 << -JXSML__TF_UNKNOWN | 1 << -JXSML__TF_LINEAR |
						1 << -JXSML__TF_SRGB | 1 << -JXSML__TF_PQ | 1 << -JXSML__TF_DCI |
						1 << -JXSML__TF_HLG
					) >> -im->gamma_or_tf & 1, "tfn?");
				}
				im->render_intent = jxsml__enum(st);
				JXSML__SHOULD((
					1 << JXSML__INTENT_PERC | 1 << JXSML__INTENT_REL |
					1 << JXSML__INTENT_SAT | 1 << JXSML__INTENT_ABS
		...
		if (extra_fields) {
			if (!jxsml__u(st, 1)) { // ToneMapping.all_default
				...
				im->intensity_target = jxsml__f16(st);
				JXSML__SHOULD(im->intensity_target > 0, "tone");
				im->min_nits = jxsml__f16(st);
				JXSML__SHOULD(0 < im->min_nits && im->min_nits <= im->intensity_target, "tone");
				relative_to_max_display = jxsml__u(st, 1);
				im->linear_below = jxsml__f16(st);
				if (relative_to_max_display) {
					JXSML__SHOULD(0 <= im->linear_below && im->linear_below <= 1, "tone");
					im->linear_below *= -1.0f;
				} else {
					JXSML__SHOULD(0 <= im->linear_below, "tone");
				}
			}
		}
	...
	if (!jxsml__u(st, 1)) { // !default_m
		int32_t cw_mask;
		if (im->xyb_encoded) {
			for (i = 0; i < 3; ++i) for (j = 0; j < 3; ++j) im->opsin_inv_mat[i][j] = jxsml__f16(st);
			for (i = 0; i < 3; ++i) im->opsin_bias[i] = jxsml__f16(st);
			for (i = 0; i < 3; ++i) im->quant_bias[i] = jxsml__f16(st);
			im->quant_bias_num = jxsml__f16(st);
		}
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_IMAGE_HEADER && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// ICC

#if defined JXSML__PART_ICC && JXSML__RECURSING < 0
...
JXSML_STATIC jxsml_err jxsml__icc(jxsml__st *st) {
	...
	for (index = 0; index < enc_size; ++index) {
		...
		//printf("%zd/%zd: %zd ctx=%d byte=%#x %c\n", index, enc_size, jxsml__bits_read(st), ctx, (int)byte, 0x20 <= byte && byte < 0x7f ? byte : ' '); fflush(stdout);
		JXSML__RAISE_DELAYED();
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_ICC && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// MA tree

#if defined JXSML__PART_MODULAR_TREE && JXSML__RECURSING < 0
...
JXSML_STATIC jxsml_err jxsml__tree(jxsml__st *st, jxsml__tree_t **tree, jxsml__code_spec_t *codespec) {
	...
	while (nodes_left-- > 0) { // depth-first, left-to-right ordering
		...
		if (prop > 0) {
			n->branch.prop = -prop;
			n->branch.value = jxsml__unpack_signed(jxsml__code(st, 0, 0, &code));
			n->branch.leftoff = ++nodes_left;
			n->branch.rightoff = ++nodes_left;
		} else {
			n->leaf.ctx = ctx_id++;
			n->leaf.predictor = jxsml__code(st, 2, 0, &code);
			n->leaf.offset = jxsml__unpack_signed(jxsml__code(st, 3, 0, &code));
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_MODULAR_TREE && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// modular header

#if defined JXSML__PART_MODULAR_HEADER && JXSML__RECURSING < 0
...
JXSML_STATIC void jxsml__combine_modular_from_pass_group(
...
JXSML_STATIC jxsml_err jxsml__init_modular_for_global(
	...
	if (frame_is_modular) { // SPEC the condition is negated
		m->num_channels += (!frame_do_ycbcr && !im->xyb_encoded && im->cspace == JXSML__CS_GREY ? 1 : 3);
	}
...
JXSML_STATIC jxsml_err jxsml__is_modular_channel_equal_sized(
	...
	while (++begin < end) {
		...
		JXSML__ASSERT(
			c.vshift < 0 || c.hshift < 0 || begin->vshift < 0 || begin->hshift < 0 ||
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_MODULAR_HEADER && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// modular prediction

#if defined JXSML__PART_MODULAR_PREDICT && JXSML__RECURSING < 0
...
#endif // defined JXSML__PART_MODULAR_PREDICT && JXSML__RECURSING < 0
#if JXSML__RECURSING == 200
...
JXSML_INLINE void jxsml__(wp_before_predict,2P)(jxsml__(wp,2P) *wp, int32_t x, int32_t y, jxsml__(neighbors_t,P) *p);
...
JXSML_STATIC jxsml_err jxsml__(modular_channel,P)(jxsml__st *st, jxsml__modular_t *m, int32_t cidx, int32_t sidx);
...
JXSML_STATIC jxsml_err jxsml__(modular_channel,P)(
	...
	for (y = 0; y < height; ++y) {
		...
		for (x = 0; x < width; ++x) {
			...
			//printf("%d ", val);
			val = jxsml__unpack_signed((int32_t) val) * n->leaf.multiplier + n->leaf.offset;
...
#if defined JXSML__PART_MODULAR_TRANSFORM && JXSML__RECURSING < 0
...
#endif // defined JXSML__PART_MODULAR_TRANSFORM && JXSML__RECURSING < 0
#if JXSML__RECURSING == 300
	#define jxsml__intP_t JXSML__CONCAT3(int, JXSML__P, _t)
	#define jxsml__int2P_t JXSML__CONCAT3(int, JXSML__2P, _t)
// ----------------------------------------

JXSML_STATIC void jxsml__(inverse_rct,P)(jxsml__st *st, jxsml__modular_t *m, const jxsml__transform_t *tr);
JXSML_STATIC jxsml_err jxsml__(inverse_palette,P)(jxsml__st *st, jxsml__modular_t *m, const jxsml__transform_t *tr);

#ifdef JXSML_IMPLEMENTATION

JXSML_STATIC void jxsml__(inverse_rct,P)(
...
JXSML_STATIC jxsml_err jxsml__inverse_transform(jxsml__st *st, jxsml__modular_t *m) {
	...
	if (st->image->modular_16bit_buffers) {
		for (i = m->nb_transforms - 1; i >= 0; --i) {
			...
			switch (tr->tr) {
			case JXSML__TR_RCT: jxsml__inverse_rct16(st, m, tr); break;
		...
		for (i = m->nb_transforms - 1; i >= 0; --i) {
			const jxsml__transform_t *tr = &m->transform[i];
			switch (tr->tr) {
			case JXSML__TR_RCT: jxsml__inverse_rct32(st, m, tr); break;
...
#if defined JXSML__PART_DEQUANT_MATRIX && JXSML__RECURSING < 0
...
JXSML_STATIC jxsml_err jxsml__load_dq_matrix(jxsml__st *st, int32_t idx, jxsml__dq_matrix_t *dqmat) {
	...
	case JXSML__DQ_ENC_AFV:
		...
		for (c = 0; c < 3; ++c) {
			...
			// access in jxsml__interpolate; libjxl avoids this by adding 1e-6 to the denominator
...
JXSML_STATIC jxsml_err jxsml__natural_order(jxsml__st *st, int32_t log_rows, int32_t log_columns, int32_t **out) {
	...
	JXSML__ASSERT(8 >= log_columns && log_columns >= log_rows && log_rows >= 3);
	...
	for (key1 = 1 << (log_columns - 3); o < size; ++key1) {
		...
		if (x1 >= (1 << log_columns)) {
			int32_t excess = jxsml__ceil_div32(x1 - ((1 << log_columns) - 1), 1 << log_slope);
			x1 -= excess << log_slope;
			y1 += excess;
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_DEQUANT_MATRIX && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// frame header & TOC

#if defined JXSML__PART_FRAME_HEADER && JXSML__RECURSING < 0
...
typedef struct jxsml__frame_st {
	int is_last;
...
JXSML_STATIC jxsml_err jxsml__frame_header(jxsml__st *st);
JXSML_STATIC jxsml_err jxsml__permutation(
	jxsml__st *st, jxsml__code_t *code, int32_t size, int32_t skip, int32_t **out
);
JXSML_INLINE void jxsml__apply_permutation(void *targetbuf, void *temp, size_t elemsize, const int32_t *lehmer);
JXSML_STATIC jxsml_err jxsml__toc(jxsml__st *st);

#ifdef JXSML_IMPLEMENTATION

JXSML_STATIC jxsml_err jxsml__frame_header(jxsml__st *st) {
	jxsml__image_st *im = st->image;
	jxsml__frame_st *f = st->frame;
	int32_t i, j;
	...
	JXSML__TRY(jxsml__zero_pad_to_byte(st));
	printf("frame starts at %d\n", (int32_t) jxsml__bits_read(st)); fflush(stdout);
	...
	if (!jxsml__u(st, 1)) { // !all_default
		...
		if (!im->xyb_encoded) f->do_ycbcr = jxsml__u(st, 1);
		if (!f->use_lf_frame) {
			...
			JXSML__SHOULD(f->ec_log_upsampling = malloc(sizeof(int32_t) * (size_t) im->num_extra_channels), "!mem");
			for (i = 0; i < im->num_extra_channels; ++i) {
		...
		if (f->type == JXSML__FRAME_LF) {
			...
			if (f->type != JXSML__FRAME_REFONLY) { // SPEC missing UnpackSigned
				f->x0 = jxsml__unpack_signed(jxsml__u32(st, 0, 8, 256, 11, 2304, 14, 18688, 30));
				f->y0 = jxsml__unpack_signed(jxsml__u32(st, 0, 8, 256, 11, 2304, 14, 18688, 30));
			}
		...
		if (f->type == JXSML__FRAME_REGULAR || f->type == JXSML__FRAME_REGULAR_SKIPPROG) {
			JXSML__SHOULD(f->ec_blend_info = malloc(sizeof(jxsml__blend_info) * (size_t) im->num_extra_channels), "!mem");
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_FRAME_HEADER && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// DCT

#if defined JXSML__PART_DCT && JXSML__RECURSING < 0

// both use `in` as a scratch space as well, so `in` will be altered after return
JXSML_STATIC void jxsml__forward_dct_unscaled(
	float *JXSML_RESTRICT out, float *JXSML_RESTRICT in, int32_t t, int32_t rep
);
JXSML_STATIC void jxsml__inverse_dct(
	float *JXSML_RESTRICT out, float *JXSML_RESTRICT in, int32_t t, int32_t rep
);

JXSML_STATIC void jxsml__forward_dct2d_scaled_for_llf(
	float *JXSML_RESTRICT buf, float *JXSML_RESTRICT scratch, int32_t log_rows, int32_t log_columns
);
JXSML_STATIC void jxsml__inverse_dct2d(
	float *JXSML_RESTRICT buf, float *JXSML_RESTRICT scratch, int32_t log_rows, int32_t log_columns
);

JXSML_STATIC void jxsml__inverse_dct11(float *buf);
JXSML_STATIC void jxsml__inverse_dct22(float *buf);
JXSML_STATIC void jxsml__inverse_hornuss(float *buf);
JXSML_STATIC void jxsml__inverse_dct32(float *buf);
JXSML_STATIC void jxsml__inverse_dct23(float *buf);
JXSML_STATIC void jxsml__inverse_afv22(float *JXSML_RESTRICT out, float *JXSML_RESTRICT in);
JXSML_STATIC void jxsml__inverse_afv(float *buf, int flipx, int flipy);
...
JXSML_STATIC const float JXSML__HALF_SECANTS[256] = {
	0, 0, // unused
...
// older versions of libjxl (both the current libjxl and JXSML currently uses Perera-Liu) and
// not even a resampling algorithm to begin with.
//
// [(1<<N) + k] = 1 / (cos(k/2^(4+N) pi) * cos(k/2^(3+N) pi) * cos(k/2^(2+N) pi) * 2^N)
//                for N >= 1 and 0 <= k < 2^N
JXSML_STATIC const float JXSML__LF2LLF_SCALES[64] = {
	0, // unused
	1.00000000f, // N=1, n=8
...
JXSML_STATIC void jxsml__forward_dct_recur_x8(JXSML__DCT_ARGS, int32_t rep1, int32_t rep2) {
	JXSML__ASSERT(rep2 == 8); (void) rep2;
	if (t < 4) {
		JXSML__ASSERT(t == 3);
...
JXSML_STATIC void jxsml__forward_dct_unscaled(JXSML__DCT_ARGS, int32_t rep) {
	if (t <= 0) {
		...
		jxsml__forward_dct_recur_x8(out, in, t, rep / 8, 8);
	} else {
		if (t == 1) return jxsml__dct2(out, in, 1, rep, 1);
		if (t == 2) return jxsml__forward_dct4(out, in, 2, rep, 1);
		jxsml__forward_dct_recur(out, in, t, rep, 1);
	}
}

JXSML_ALWAYS_INLINE void jxsml__forward_dct_unscaled_view(jxsml__view_f32 *outv, jxsml__view_f32 *inv) {
	jxsml__adapt_view_f32(outv, inv->logw, inv->logh);
	jxsml__forward_dct_unscaled(outv->ptr, inv->ptr, inv->logh, 1 << inv->logw);
}
...
JXSML_ALWAYS_INLINE void jxsml__inverse_dct_view(jxsml__view_f32 *outv, jxsml__view_f32 *inv) {
	jxsml__adapt_view_f32(outv, inv->logw, inv->logh);
	jxsml__inverse_dct(outv->ptr, inv->ptr, inv->logh, 1 << inv->logw);
}
...
JXSML_STATIC void jxsml__forward_dct2d_scaled_for_llf(
	...
	jxsml__view_f32 bufv = jxsml__make_view_f32(log_columns, log_rows, buf);
	jxsml__view_f32 scratchv = jxsml__make_view_f32(log_columns, log_rows, scratch);
	float *p;
	int32_t x, y;

	jxsml__forward_dct_unscaled_view(&scratchv, &bufv);
	jxsml__transpose_view_f32(&bufv, scratchv);
	jxsml__forward_dct_unscaled_view(&scratchv, &bufv);
	...
	JXSML__VIEW_FOREACH(scratchv, y, x, p) {
		// hopefully compiler will factor the second multiplication out of the inner loop (TODO verify this)
		*p *= JXSML__LF2LLF_SCALES[(1 << scratchv.logw) + x] * JXSML__LF2LLF_SCALES[(1 << scratchv.logh) + y];
	}
	...
	if (log_columns > log_rows) {
		...
		jxsml__copy_view_f32(&bufv, scratchv);
	}
	JXSML__ASSERT(bufv.logw == jxsml__max32(log_columns, log_rows));
	JXSML__ASSERT(bufv.logh == jxsml__min32(log_columns, log_rows));
}

JXSML_STATIC void jxsml__inverse_dct2d(
	float *JXSML_RESTRICT buf, float *JXSML_RESTRICT scratch, int32_t log_rows, int32_t log_columns
) {
	jxsml__view_f32 bufv;
	jxsml__view_f32 scratchv = jxsml__make_view_f32(log_columns, log_rows, scratch);

	if (log_columns > log_rows) {
		// TODO spec improvement: coefficients start being transposed, note this as well
		bufv = jxsml__make_view_f32(log_columns, log_rows, buf);
		jxsml__transpose_view_f32(&scratchv, bufv);
	} else {
		bufv = jxsml__make_view_f32(log_rows, log_columns, buf);
		jxsml__copy_view_f32(&scratchv, bufv);
	}
	jxsml__inverse_dct_view(&bufv, &scratchv);
	jxsml__transpose_view_f32(&scratchv, bufv);
	jxsml__inverse_dct_view(&bufv, &scratchv);
	JXSML__ASSERT(bufv.logw == log_columns && bufv.logh == log_rows);
}

// a single iteration of AuxIDCT2x2
JXSML_ALWAYS_INLINE void jxsml__aux_inverse_dct11(float *out, float *in, int32_t x, int32_t y, int32_t S2) {
...
JXSML_STATIC void jxsml__inverse_dct11(float *buf) {
	...
	jxsml__aux_inverse_dct11(buf, buf, 0, 0, 1); // updates buf[(0..1)*8+(0..1)]
	// updates scratch[(0..3)*8+(0..3)], copying other elements from buf in verbatim
	memcpy(scratch, buf, sizeof(float) * 64);
	for (y = 0; y < 2; ++y) for (x = 0; x < 2; ++x) jxsml__aux_inverse_dct11(scratch, buf, x, y, 2);
	// updates the entire buf
	for (y = 0; y < 4; ++y) for (x = 0; x < 4; ++x) jxsml__aux_inverse_dct11(buf, scratch, x, y, 4);
}
...
JXSML_STATIC void jxsml__inverse_dct22(float *buf) {
	...
	jxsml__aux_inverse_dct11(buf, buf, 0, 0, 1);
	...
	jxsml__inverse_dct(scratch, buf, 2, 16); // columnar IDCT for a#-m#, b#-n#, c#-o# and d#-p#
	for (y = 0; y < 8; ++y) for (x = 0; x < 8; ++x) buf[x * 8 + y] = scratch[y * 8 + x];
	jxsml__inverse_dct(scratch, buf, 2, 16); // columnar IDCT for a#-d#, e#-h#, i#-l# and m#-p#
...
JXSML_STATIC void jxsml__inverse_hornuss(float *buf) {
	...
	int32_t x, y, ix, iy;
	memcpy(scratch, buf, sizeof(float) * 64);
	jxsml__aux_inverse_dct11(scratch, buf, 0, 0, 1); // updates scratch[(0..1)*8+(0..1)]
	for (y = 0; y < 2; ++y) for (x = 0; x < 2; ++x) {
		int32_t pos00 = y * 8 + x, pos11 = (y + 2) * 8 + (x + 2);
		float rsum[4] = {}, sample11;
		for (iy = 0; iy < 4; ++iy) {
			for (ix = 0; ix < 4; ++ix) rsum[ix] += scratch[(y + iy * 2) * 8 + (x + ix * 2)];
		}
...
JXSML_STATIC void jxsml__inverse_dct32(float *buf) {
	...
	jxsml__view_f32 bufv = jxsml__make_view_f32(3, 3, buf);
	jxsml__view_f32 scratchv = jxsml__make_view_f32(3, 3, scratch);
	...
	tmp = *JXSML__AT(bufv, 0, 0) + *JXSML__AT(bufv, 0, 1);
	*JXSML__AT(bufv, 0, 1) = *JXSML__AT(bufv, 0, 0) - *JXSML__AT(bufv, 0, 1);
	*JXSML__AT(bufv, 0, 0) = tmp;
	jxsml__reshape_view_f32(&bufv, 4, 2);
	jxsml__inverse_dct_view(&scratchv, &bufv);
	jxsml__reshape_view_f32(&scratchv, 3, 3);
	jxsml__transpose_view_f32(&bufv, scratchv);
	jxsml__inverse_dct_view(&scratchv, &bufv);
	jxsml__oddeven_columns_to_halves_f32(&bufv, scratchv);
	JXSML__ASSERT(bufv.logw == 3 && bufv.logh == 3);
}
...
JXSML_STATIC void jxsml__inverse_dct23(float *buf) {
	...
	// coefficients form two 4 rows x 8 columns matrices from even and odd rows
	jxsml__copy_view_f32(&scratchv, bufv);
	*JXSML__AT(scratchv, 0, 0) = *JXSML__AT(bufv, 0, 0) + *JXSML__AT(bufv, 0, 1);
	*JXSML__AT(scratchv, 0, 1) = *JXSML__AT(bufv, 0, 0) - *JXSML__AT(bufv, 0, 1);
	jxsml__transpose_view_f32(&bufv, scratchv);
	jxsml__inverse_dct_view(&scratchv, &bufv);
	jxsml__transpose_view_f32(&bufv, scratchv);
	jxsml__reshape_view_f32(&bufv, 4, 2);
	jxsml__inverse_dct_view(&scratchv, &bufv);
	jxsml__reshape_view_f32(&scratchv, 3, 3);
	jxsml__oddeven_rows_to_halves_f32(&bufv, scratchv);
	JXSML__ASSERT(bufv.logw == 3 && bufv.logh == 3);
}
...
JXSML_STATIC void jxsml__inverse_afv(float *buf, int flipx, int flipy) {
	...
	JXSML__ASSERT(flipx == !!flipx && flipy == !!flipy);
	...
	jxsml__inverse_afv22(bufafv, scratchafv);
	jxsml__inverse_dct(buf22, scratch22, 2, 4);
	jxsml__inverse_dct(buf32, scratch32, 3, 4);
	...
	jxsml__inverse_dct(buf22, scratch22, 2, 4);
	jxsml__inverse_dct(buf23, scratch23, 2, 8);
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_DCT && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// LfGlobal: additional image features, HF block context, global tree, extra channels

#if defined JXSML__PART_LF_GLOBAL && JXSML__RECURSING < 0

JXSML_STATIC jxsml_err jxsml__lf_global(jxsml__st *st);
		...
		if (jxsml__u(st, 1)) {
			...
			for (i = 0; i < 3; ++i) {
				...
				for (j = 0; j < f->nb_lf_thr[i]; ++j) {
					f->lf_thr[i][j] = jxsml__unpack_signed(jxsml__u32(st, 0, 4, 16, 8, 272, 16, 65808, 32));
				}
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_LF_GLOBAL && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// LfGroup: downsampled LF image (optionally smoothed), varblock information

#if defined JXSML__PART_LF_GROUP && JXSML__RECURSING < 0
...
JXSML_STATIC jxsml_err jxsml__lf_quant(
	jxsml__st *st, int32_t extra_prec, jxsml__modular_t *m, jxsml__lf_group_t *gg, float (**outlfquant)[3]
);
JXSML_STATIC jxsml_err jxsml__hf_metadata(
	jxsml__st *st, int32_t nb_varblocks,
	jxsml__modular_t *m, const float (*lfquant)[3], jxsml__lf_group_t *gg
);
JXSML_STATIC jxsml_err jxsml__lf_group(
	jxsml__st *st, int32_t ggw, int32_t ggh, int32_t ggidx, jxsml__lf_group_t *gg
);
...
JXSML_STATIC jxsml_err jxsml__lf_quant(
	...
	for (c = 0; c < 3; ++c) {
		...
		mult_lf[c] = f->m_lf_scaled[c] / (float) (f->global_scale * f->quant_lf) * (float) (65536 >> extra_prec);
	}
...
JXSML_STATIC jxsml_err jxsml__hf_metadata(
	...
	float *coeffs[3 /*xyb*/] = {}, *llfcoeffs[3 /*xyb*/] = {};
	...
	for (c = 0; c < 3; ++c) { // TODO account for chroma subsampling
		JXSML__SHOULD(coeffs[c] = calloc((size_t) (ggw8 * ggh8 * 64), sizeof(float)), "!mem");
		JXSML__SHOULD(llfcoeffs[c] = malloc((size_t) (ggw8 * ggh8) * sizeof(float)), "!mem");
	}
...
JXSML_STATIC jxsml_err jxsml__lf_group(
	...
	if (!f->is_modular) {
		...
		if (!f->use_lf_frame) {
			...
			JXSML__TRY(jxsml__lf_quant(st, extra_prec, &m, gg, &lfquant));
		...
		JXSML__TRY(jxsml__finish_and_free_code(st, &m.code));
		JXSML__TRY(jxsml__inverse_transform(st, &m));
		JXSML__TRY(jxsml__hf_metadata(st, nb_varblocks, &m, lfquant, gg));
		jxsml__free_modular(&m);
		free(lfquant);
		lfquant = NULL;
	}
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_LF_GROUP && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// HfGlobal and HfPass

#if defined JXSML__PART_HF_GLOBAL && JXSML__RECURSING < 0

JXSML_STATIC jxsml_err jxsml__hf_global(jxsml__st *st);
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_HF_GLOBAL && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// PassGroup

#if defined JXSML__PART_PASS_GROUP && JXSML__RECURSING < 0

JXSML_STATIC jxsml_err jxsml__hf_coeffs(
	jxsml__st *st, int32_t ctxoff, int32_t pass,
	int32_t gx_in_gg, int32_t gy_in_gg, int32_t gw, int32_t gh, jxsml__lf_group_t *gg
);
JXSML_STATIC jxsml_err jxsml__pass_group(
	jxsml__st *st, int32_t pass, int32_t gx_in_gg, int32_t gy_in_gg, int32_t gw, int32_t gh, int32_t gidx,
...
JXSML_STATIC jxsml_err jxsml__hf_coeffs(
	...
	const jxsml__frame_st *f = st->frame;
		...
		for (x8 = 0; x8 < gw8; ++x8) {
			...
			for (c_yxb = 0; c_yxb < 3; ++c_yxb) {
				...
				for (i = 1 << (log_size - 6); nz > 0 && i < (1 << log_size); ++i) {
					...
					coeffs[order[i]] += (float) jxsml__unpack_signed(ucoeff);
...
JXSML_STATIC jxsml_err jxsml__pass_group(
	...
	if (!f->is_modular) {
		...
		JXSML__TRY(jxsml__hf_coeffs(st, ctxoff, pass, gx_in_gg, gy_in_gg, gw, gh, gg));
	}
	...
	if (m.num_channels > 0) {
		...
		JXSML__TRY(jxsml__finish_and_free_code(st, &m.code));
		JXSML__TRY(jxsml__inverse_transform(st, &m));
		jxsml__combine_modular_from_pass_group(st, f->num_gm_channels,
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_PASS_GROUP && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// coefficients to samples

#if defined JXSML__PART_DEQUANT && JXSML__RECURSING < 0

JXSML_STATIC void jxsml__dequant_hf(jxsml__st *st, jxsml__lf_group_t *gg);
JXSML_STATIC jxsml_err jxsml__combine_vardct_from_lf_group(
	jxsml__st *st, int32_t ggx, int32_t ggy, const jxsml__lf_group_t *gg
);
...
JXSML_STATIC void jxsml__dequant_hf(jxsml__st *st, jxsml__lf_group_t *gg) {
	...
	for (i = 0; i < nblocks; ++i) {
		...
		for (c = 0; c < 3; ++c) {
			...
			for (j = 0; j < size; ++j) { // LLF positions are left unused and can be clobbered
				...
				coeffs[j] *= mult[c] / dqmat->params[j][c]; // TODO precompute this
			}
		}
	}
}
...
JXSML_STATIC jxsml_err jxsml__combine_vardct_from_lf_group(
	...
	for (y8 = 0; y8 < ggh8; ++y8) for (x8 = 0; x8 < ggw8; ++x8) {
		...
		vh8 = 1 << (jxsml__min32(dct->log_rows, dct->log_columns) - 3);
		vw8 = 1 << (jxsml__max32(dct->log_rows, dct->log_columns) - 3);
		...
		for (c = 0; c < 3; ++c) {
			...
			// inverse DCT
			switch (dctsel) {
			case 1: jxsml__inverse_hornuss(scratch); break; // Hornuss
			case 2: jxsml__inverse_dct11(scratch); break; // DCT11
			case 3: jxsml__inverse_dct22(scratch); break; // DCT22
			case 12: jxsml__inverse_dct23(scratch); break; // DCT23
			case 13: jxsml__inverse_dct32(scratch); break; // DCT32
			case 14: jxsml__inverse_afv(scratch, 0, 0); break; // AFV0
			case 15: jxsml__inverse_afv(scratch, 1, 0); break; // AFV1
			case 16: jxsml__inverse_afv(scratch, 0, 1); break; // AFV2
			case 17: jxsml__inverse_afv(scratch, 1, 1); break; // AFV3
	...
	if (im->modular_16bit_buffers) {
		...
		for (y = 0; y < ggh; ++y) for (x = 0; x < ggw; ++x) {
			int32_t p = y * ggw + x, q = (ggy + y) * f->width + (ggx + x);
			for (c = 0; c < 3; ++c) {
				float v = 
				...
				// TODO overflow check
				pixels[c][q] = (int16_t) ((float) ((1 << im->bpp) - 1) * v + 0.5f);
			}
		}
	} else {
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_DEQUANT && JXSML__RECURSING < 0

////////////////////////////////////////////////////////////////////////////////
// frame

#if defined JXSML__PART_FRAME && JXSML__RECURSING < 0

JXSML_STATIC jxsml_err jxsml__frame(jxsml__st *st);
...
JXSML_STATIC jxsml_err jxsml__frame(jxsml__st *st) {
	...
	JXSML__TRY(jxsml__frame_header(st));
	JXSML__TRY(jxsml__toc(st));
	...
	JXSML__TRY(jxsml__lf_global(st));
	...
	// LfGroups
	{
		...
		for (ggy = 0; ggy < f->height; ggy += ggsize) {
			...
			for (ggx = 0; ggx < f->width; ggx += ggsize, ++ggidx) {
				...
				JXSML__TRY(jxsml__lf_group(st, ggw, ggh, ggidx, f->is_modular ? NULL : &gg[ggidx]));
				if (!single_section) JXSML__TRY(jxsml__zero_pad_to_byte(st));
			}
		}
	}
	...
	if (!f->is_modular) {
		JXSML__TRY(jxsml__hf_global(st));
		if (!single_section) JXSML__TRY(jxsml__zero_pad_to_byte(st));
	}
	...
	for (i = 0; i < f->num_passes; ++i) {
		...
		for (gy = 0; gy < f->height; gy += gsize) {
			...
			for (gx = 0; gx < f->width; gx += gsize, ++gidx) {
				...
				JXSML__TRY(jxsml__pass_group(st, i, gx - ggx, gy - ggy, gw, gh, gidx, ggx, ggy,
	...
	if (!f->is_modular) {
		...
		JXSML__SHOULD(!f->do_ycbcr && st->image->cspace != JXSML__CS_GREY, "TODO: we don't yet do YCbCr or gray");
		JXSML__SHOULD(st->image->modular_16bit_buffers, "TODO: !modular_16bit_buffers");
		...
		for (ggy = 0; ggy < f->height; ggy += ggsize) {
			for (ggx = 0; ggx < f->width; ggx += ggsize, ++ggidx) {
				jxsml__dequant_hf(st, &gg[ggidx]);
				JXSML__TRY(jxsml__combine_vardct_from_lf_group(st, ggx, ggy, &gg[ggidx]));
			}
		}
	}
...
#endif // defined JXSML_IMPLEMENTATION
#endif // defined JXSML__PART_FRAME && JXSML__RECURSING < 0
...
const char *dumppath;
void end(const jxsml__st st);

jxsml_err jxsml_from_file(const char *path);

#ifdef JXSML_IMPLEMENTATION

const char *dumppath = NULL;
void end(const jxsml__st st) { (void) st; }
...
jxsml_err jxsml_from_file(const char *path) {
	jxsml__source src = {0};
	jxsml__container_st container = {0};
	jxsml__image_st im = {
	...
	jxsml__st st_ = { .source = &src, .container = &container, .image = &im };
	jxsml__st *st = &st_;
	int32_t i, j, k;

	JXSML__TRY(jxsml__init_file_source(st, path, 8 * 1024 * 1024, st->source));
	JXSML__TRY(jxsml__refill_backing_buffer(st));

	JXSML__TRY(jxsml__container(st));

	JXSML__SHOULD(st->remaining >= 2, "shrt");
	JXSML__SHOULD(st->ptr[0] == 0xff && st->ptr[1] == 0x0a, "!jxl");
	st->ptr += 2;
	st->remaining -= 2;

	JXSML__TRY(jxsml__size_header(st, &im.width, &im.height));
	JXSML__TRY(jxsml__image_metadata(st));
	if (im.want_icc) JXSML__TRY(jxsml__icc(st));

	jxsml__frame_st frame;
	do {
		st->frame = &frame;
		JXSML__TRY(jxsml__frame(st));
		...
		if (dumppath && frame.type == JXSML__FRAME_REGULAR) {
			JXSML__ASSERT(im.modular_16bit_buffers && im.bpp >= 8 && im.exp_bits == 0);
			FILE *f = fopen(dumppath, "wb");
			uint32_t crc, adler, unused = 0, idatsize;
			int16_t *c[4];
			int32_t nchan;
			uint8_t buf[32];
			nchan = (!frame.do_ycbcr && !im.xyb_encoded && im.cspace == JXSML__CS_GREY ? 1 : 3);
			for (i = 0; i < nchan; ++i) c[i] = (void*) frame.gmodular.channel[i].pixels;
			for (i = nchan; i < frame.gmodular.num_channels; ++i) {
				jxsml__ec_info *ec = &im.ec_info[i - nchan];
				if (ec->type == JXSML__EC_ALPHA) {
					JXSML__ASSERT(ec->bpp == im.bpp && ec->exp_bits == im.exp_bits);
			...
			for (i = 0; i < frame.height; ++i) {
				...
				for (j = 0; j < frame.width; ++j) {
					for (k = 0; k < nchan; ++k) {
						int32_t p = jxsml__min32(jxsml__max32(0, c[k][i * frame.width + j]), (1 << im.bpp) - 1);
